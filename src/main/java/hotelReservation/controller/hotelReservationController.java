...
public class hotelReservationController {
	...
	public String createCustomer (@Valid Customer customerInfo, BindingResult result, Model model)
	{
		if (result.hasErrors()) {
			...
			model.addAttribute("dateTomorrow", dateTomorrow);
	...
	public String displayReservation(@RequestParam(defaultValue = "abcdefghijk@lmnopqrstuvwzyz.com") String reservationEmail,
			Model model) {
	...
	public String cancelOrEdit(
		...
		if (action.equalsIgnoreCase("Cancel")) {
			...
			customerInfo = reservationService.getCustomerByID(customerID);
			SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
			Date now = new Date();
			String dateNow = formatter.format(now);
			Calendar calendar = Calendar.getInstance();
